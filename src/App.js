...
class App extends React.Component {
  ...
  onJokeButton(event) {
    ...
    const endpoint = `https://api.icndb.com/jokes/random/${
      this.state.count
    }?exclude=[explicit]&escape=javascript${addFirstname}${addLastname}${limitToDev}`;
    ...
    return (
      <div className="App">
        ...
        <div className="menu">
          ...
          <button onClick={this.onShowSettings} tabIndex="1">
            Settings{" "}
            {this.state.visible ? (
        ...
        {this.state.result !== "" && <FactDisplay result={this.state.result} />}
        {this.state.result !== "" && (
          <div>
            <button onClick={this.onJokeButton} tabIndex="3" name="more">
              Give me {this.state.count} more!
            </button>
            <button onClick={this.onClearButton} tabIndex="3">
              Clear all!
            </button>
          </div>
        )}
